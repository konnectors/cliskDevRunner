...
export class WorkerService extends EventEmitter {
  ...
  async handleUrlChange(newUrl, oldUrl) {
    ...
    if (
      !this.currentReconnectionPromise ||
      this.currentReconnectionPromise.settled
    ) {
    ...
    try {
      if (
        !page.context() ||
        (page.context().isConnected && !page.context().isConnected())
      ) {
        this.navLog(
          "⚠️ Browser context is disconnected, skipping auto-reconnection"
        );
      ...
      this.navLog(
        "⚠️ Cannot access browser context, skipping auto-reconnection: %O",
        error
      );
    ...
    const reconnectionTimeout = this.createTrackedTimeout(() => {
      ...
      this.reconnectionReject(
        new Error("Reconnection timeout after 15 seconds")
      );
    }, 15000);
    ...
    try {
      ...
      if (!page || page.isClosed()) {
        ...
        this.reconnectionReject(
          new Error("Page was closed during reconnection")
        );
        return false;
      }
      ...
      try {
        ...
        if (
          ...
          this.navLog(
            "⚠️ Page closed during stabilization, aborting reconnection"
          );
          this.clearTrackedTimeout(reconnectionTimeout);
          this.reconnectionReject(
            new Error("Page closed during stabilization")
          );
          return false;
        }
        throw error;
      ...
      if (this.workerPage.connectorPath && this.workerPage.loaderFunction) {
        ...
        try {
          await this.workerPage.loaderFunction(
            page,
            this.workerPage.connectorPath
          );
        ...
        this.navLog(
          "⚠️ No connector path or loader function available for re-injection"
        );
      }
      ...
      const isPageClosedError =
        ...
        (error.message.includes(
          ...
          error.message.includes(
            "Page was closed during handshake initialization"
          ) ||
          error.message.includes(
            "Page was closed before handshake could complete"
          ));

      if (isPageClosedError) {
        this.navLog(
          "ℹ️ Reconnection cancelled due to page closure (normal during cleanup)"
        );
        this.reconnectionReject(
          new Error("Reconnection cancelled due to page closure")
        );
      } else {
